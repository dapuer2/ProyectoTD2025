...
p_load(
  ...
  gt,# Para la visualización de tablas
  flextable,,# Para la visualización de tablas
  officer,# Para la visualización de tablas
  scales # Manipulación de gráficas
  )
```
...
df.productos <- df.productos %>%
  mutate(categoria = case_when(
    ...
    # PESCADO
    nombre.producto %in% c("MEJILLON", "PATÉ ATÚN 3X80", "BERBERECHO", "LOMO BACAL P/S", 
                           "ESCALOPIN SALMON", "BACALAO AHUMADO", "ATÚN CLARO OLIVA PK6", "PORCIONES BACALAO
") ~ "PESCADO",
...
resumen_variables <- resumen %>%
  mutate(
    ...
    topFrac = scales::percent(topFrac, accuracy = 0.1),
    missFrac = scales::percent(missFrac, accuracy = 0.1)
  ) %>%
  kable(
    caption = "Resumen General de Variables",
    booktabs = TRUE,
    align = c("l", "l", "c", "l", "c", "c", "c"),
    col.names = c("Variable", "Tipo", "Niveles Ún.", "Valor Más Frec.", 
                  "Frec.", "% Total", "Faltantes (%)")
  ) %>%
  kable_styling(
...
```{r,echo=FALSE, grafico, fig.width=4, fig.height=3.4, out.width="70%", fig.align="center"}
...
```{r, echo = FALSE,grafico2, fig.width=7, fig.height=2.7,fig.align="center"}
...
```{r,echo=FALSE,grafico3, fig.width=6, fig.height=2.7,fig.align="center", out.width="70%"}

# 1) Filtramos los productos de la categoría PESCADO, agrupamos por producto y sumamos el total de kilos vendidos
top_pescado <- df.productos %>%
  filter(categoria == "PESCADO") %>% # Filtramos la categoria PESCADO
...
# 2) Representamos los resultados en un gráfico de barras
...
```{r, echo=FALSE,message=FALSE,warning=FALSE,grafico4,fig.width=5, fig.height=3,fig.align="center"}
...
```{r,fig.align="center",echo=FALSE,grafico5, fig.width=7.7, fig.height=2}
...
Valencia claramente lidera con 130 tickets. Alboraya aparece en segundo lugar con 50 tickets.
    ...
    6)  Muestra mediante un diagrama el número de tickets recogidos cada
        ...
        qué día lo harías?
```{r,fig.align="center",echo=FALSE,grafico6,fig.width=4,fig.height=2}
...
```{r,fig.align="center",echo=FALSE,grafico7,fig.width=5,fig.height=4}
...
```{r,fig.align="center",echo=FALSE,grafico8,fig.width=4,fig.height=3}
...
```{r,echo=FALSE,fig.align="center",grafico9,fig.width=5,fig.height=1.5,warning=FALSE}
...
ggplot(df.ticket,aes(x=1,y=iva_anyadido)) +
  geom_jitter(width=0.2,alpha=0.4,color="skyblue",size=2) +
  geom_hline(yintercept=iva_medio$iva_promedio,color="darkred",linetype="dashed",size=1)+
  labs(title = "Valores de IVA por ticket",y = "IVA (€)",x = NULL) +
  theme_minimal() +
...
```
    ...
    10) ¿Cuál es la tienda en la que más se ha comprado? (Dirección
        tienda)
```{r,fig.align="center",echo=FALSE,grafico10,fig.width=6,fig.height=2}


df.ticket%>%
  count(direccion,sort=TRUE) %>%
  slice_max(n, n = 5) %>% # Utilizamos count para contar las veces que aparece cada tienda
...
```{r,fig.align="center",echo=FALSE,grafico11,fig.width=4,fig.height=2}
...
```{r,fig.align="center",echo=FALSE,grafico12,fig.width=6,fig.height=2}
