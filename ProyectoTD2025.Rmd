...
```{r}
#Hacemos un data.frame con los datos de la tabla de arriba
top_frutas_verduras <- data.frame(
  nombre.producto = c("TOMATE TRITURADO", "NUEZ CASCARA NATURAL", 
                      "AGUACATE BANDEJA", "AJO SECO 250 G", "ESP VERDE FINO"),
  kilos_vendidos = c(12, 7, 5, 5, 5)
)

# Hacemos la gráfica
ggplot(top_frutas_verduras, aes(x = reorder(nombre.producto, -kilos_vendidos), 
                               y = kilos_vendidos)) +
  geom_col(fill = "#4CAF50", width = 0.7) +  # Usamos geom_col para valores explícitos
